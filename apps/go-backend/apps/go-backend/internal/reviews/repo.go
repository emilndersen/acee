...
const selectColumns = `id, author_name, text, rating, is_visible, created_at::text`
...
func (r *Repo) Create(ctx context.Context, in CreateReviewInput) (Review, error) {
	...
	`, in.AuthorName, in.Text, in.Rating).Scan(
		&rev.ID, &rev.AuthorName, &rev.Text, &rev.Rating, &rev.IsVisible, &rev.CreatedAt,
	)
	return rev, err
}
...
func (r *Repo) ListVisible(ctx context.Context) ([]Review, error) {
	...
	for rows.Next() {
		var rev Review
		if err := rows.Scan(&rev.ID, &rev.AuthorName, &rev.Text, &rev.Rating, &rev.IsVisible, &rev.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, rev)
	}
